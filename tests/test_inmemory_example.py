...
Note: This test uses the legacy graphiti_mcp_server.py (backward compatibility).
      Production deployments use src/server.py:create_server() factory pattern.

Requirements:
- pytest-asyncio
- Running FalkorDB instance (docker compose up -d)
